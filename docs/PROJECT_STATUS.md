...
- **SMS Notifications** (Twilio)
  - Job status updates and manual send (requires Twilio config)

#### 7. UX Enhancements
- **Toast Notifications** - Success/error feedback
- **Global Search** (Cmd+K) - Search jobs, customers, invoices
- **Bulk Selection** - Multi-select with actions
- **Loading Skeletons** - Route-level skeleton states
- **Empty States** - Illustrated CTAs for empty lists
- **Keyboard Shortcuts** - Global shortcuts and help modal
- **Context Menus** - Quick actions on rows/cards
...
1. **Subscription Billing Page** (~1 hour)
   - Wire billing UI under `/admin/settings/billing`
   - Connect Stripe billing portal actions

2. **Analytics Time-Range Consistency** (~1-2 hours)
   - Align time windows across analytics widgets
   - Add a shared range selector

#### Medium Priority (Next Month)
1. **Stripe Subscriptions Hardening** (2-3 hours)
   - Finalize product/price IDs
   - Validate webhook flows end-to-end

2. **Audit Logging** (1-2 hours)
   - Track all user actions
   - Security compliance
   - Debug support

3. **Two-Factor Authentication** (2-3 hours)
   - Enhanced security
   - QR code setup
   - Recovery codes
...
**Next Milestone:** Subscription billing page + analytics consistency
