...
## Phase 3: SMS Notifications - COMPLETE (Config required)

### What's Been Built:
1. **Twilio client and SMS service** (`lib/twilio/client.ts`, `lib/sms/client.ts`)
2. **SMS templates** (`lib/twilio/templates.ts`)
3. **Job status + tech assignment triggers** (`lib/sms/job-notifications.ts`, `app/api/jobs/update-status/route.ts`)
4. **API routes + webhook** (`app/api/sms/send/route.ts`, `app/api/sms/history/route.ts`, `app/api/sms/webhook/route.ts`)
5. **Admin settings + history UI** (`components/admin/SettingsForm.tsx`, `components/admin/SMSHistory.tsx`, `components/admin/JobDetailsAdmin.tsx`)
...
**Webhook**:
- Configure Twilio status callback to `/api/sms/webhook`

---

## 🎯 Next Steps - TO IMPLEMENT

### Phase 4: Stripe Subscription Management (partial)
**Why**: Generate recurring revenue

**What's Built**:
- `lib/stripe/client.ts`, `lib/stripe/subscriptions.ts`, `lib/stripe/utils.ts`
- API routes: `/api/subscriptions/create`, `/api/subscriptions/update-tier`, `/api/subscriptions/cancel`, `/api/subscriptions/billing-portal`
- Webhook handler: `/api/stripe/webhook`
- Subscription UI component: `components/admin/SubscriptionManager.tsx`

**Still Needed**:
- Wire a billing page (planned: `app/admin/settings/billing/page.tsx`)
- Stripe product/price IDs in env (see `docs/STRIPE_SUBSCRIPTION_SETUP.md`)
...
| SMS Notifications | Done (config required) | High | - |
| Stripe Billing | Partial | High | 2-3 hours |
...
### Coming Soon
- Subscription billing UI wiring
...
- Admin and tech notification integration
- SMS notifications (requires Twilio config)
### Ready to Add:
- Stripe subscriptions UI wiring (billing page)
